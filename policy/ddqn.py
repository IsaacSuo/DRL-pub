...
class DoubleDeepQNetworkPolicy(BasePolicy):
    ...
    def update(self, 
        ...
        action_indices = tf.stack([tf.range(tf.shape(best_actions)[0]), # eg.[0, 1, ..., 31]
                                   tf.cast(best_actions, tf.int32)], axis=1) # eg.[0, 1,..., 1]  cast_shape (32, 2)
        ...
        q_targets = tf.gather_nd(next_target_q_values, action_indices) # (32, ) eg. next[0,1] = 1.1, next[1,0] = 1.4, ...
        q_targets = rewards + (1.0 - dones) * tensor_gamma * q_targets # (32, )
        ...
        actions_taken = tf.stack([tf.range(tf.shape(actions)[0]), # eg.[0, 1, ..., 31]
                                  actions], axis=1) # cast_shape (32, 2)
        y_values = tf.gather_nd(y_values, actions_taken) # (32, )
